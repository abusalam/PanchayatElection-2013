...
<?php PE\ShowMsg(); ?>
    ...
    <form name="frmCountPP" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>"
<?php echo (PE\GetVal($_SESSION, 'Step') !== NULL) ? '' : 'style="display:none;"'; ?>>
      <hr />
      <div class="FieldGroup">
        <h3><?php echo PE\GetColHead('off_code_cp'); ?>:</h3>
        <select name="off_code_cp" onchange="document.frmCountPP.submit();">
          <?php
          $Data->show_sel("off_code", "off_desc", "select off_code, CONCAT(office, ' - ', off_code) as off_desc "
                  . "from " . MySQL_Pre . "office "
                  . "Where blockmuni='" . $_SESSION['BlockCode'] . "'", PE\GetVal($_SESSION['PostData'], 'off_code_cp', TRUE));
          ?>
        </select>
      </div>
        ...
        <div class="FieldGroup">
          ...
          <select name="PersSL">
            ...
            $Data->show_sel("PersSL", "Pofficer_nm", "select PersSL, CONCAT(officer_nm, ' - ', PersSL) as Pofficer_nm "
                    . "from " . MySQL_Pre . "count_personnel "
                    . " Where Deleted=0 AND off_code_cp='" . PE\GetVal($_SESSION['PostData'], 'off_code_cp', TRUE)
                    . "' AND OFFBLOCK_CODE='" . $_SESSION['BlockCode']
